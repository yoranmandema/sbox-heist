...
public class InventoryBar : Panel
{
	...
	List<BaseHeistWeapon> Weapons = new();

	public bool IsOpen;
	BaseHeistWeapon SelectedWeapon;
	...
	public override void Tick()
	{
		...
		Weapons.Clear();
		Weapons.AddRange( player.Children.Select( x => x as BaseHeistWeapon ).Where( x => x.IsValid() && x.IsUsable() ) );
	...
	public void ProcessClientInput( InputBuilder input )
	{
		...
		if ( IsOpen != wantOpen )
		{
			SelectedWeapon = Local.Pawn.ActiveChild as BaseHeistWeapon;
			IsOpen = true;
		}
	...
	int NextInBucket()
	{
		...
		BaseHeistWeapon first = null;
		BaseHeistWeapon prev = null;
